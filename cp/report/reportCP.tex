...
\subsection*{Предобработка связности (компоненты)}
Чтобы быстро отвечать \texttt{-1} без запуска A*, заранее вычисляются компоненты связности по свободным клеткам с помощью BFS. Если старт и финиш принадлежат разным компонентам, пути не существует.
...
\begin{itemize}
  ...
  \item \texttt{nxt} --- вершины с $f=\texttt{curF}+2$.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Предобработка:} построение массива компонент связности BFS по всему полю.
  \item \textbf{Ответы на запросы:} для каждого запроса:
    \begin{enumerate}
      \item проверка равенства компонентов старта и финиша (иначе \texttt{-1});
      \item запуск A* с манхэттенской эвристикой (и ускорением через два списка \texttt{cur/nxt}).
    \end{enumerate}
\end{itemize}
...
Итак, для запросов внутри одной компоненты связности алгоритм возвращает длину кратчайшего пути, а для разных компонент --- \texttt{-1}.
...
\begin{itemize}
  \item Предобработка компонент связности (BFS по решётке): $O(N)$ по времени и $O(N)$ по памяти.
...
\end{verbatim}
  ...
  \begin{itemize}
    ...
    \item Поле полностью заполнено препятствиями, кроме точек из запроса (гарантированно свободных): чаще всего ответ \texttt{-1}.
  \end{itemize}
\end{enumerate}
...
\begin{itemize}
  \item Добавлена предобработка компонент связности, чтобы мгновенно отвечать \texttt{-1} для запросов в разных компонентах.
...
\begin{itemize}
  \item предварительная разметка компонент связности BFS;
